  ...
  <div>
            ...
            <div class="main_box">
                <ul class="container">
                  <li class="selitems">
                      ...
                      <router-link class="navitem" to="/immigrant">
                        移民项目
                      </router-link>
                      <template v-for="(item,index) in queryData" v-if="(item>0 && index!=1) || (typeof item=='object' && item.includes(1)) || (item>=0 && index==1)">
                  ...
                  <li class="selitems" style="position: relative">
                    ...
                    <div class="sels">
                      ...
                      <a class="sel sel-auto" v-if="index<16" v-for="(item,index) in selData.countryData" @click="change(0,index+1)" :class="{'active':queryData[0]==index+1}">{{item.label}}</a>
                      <a class="sel sel-auto" style="margin-top: 10px" v-if="index>16 && showMoreNation" v-for="(item,index) in selData.countryData" @click="change(0,index+1)" :class="{'active':queryData[0]==index+1}">{{item.label}}</a>
                    ...
                    <!--<a class="sel fix" v-for="(item,index) in selData.stateData" @click="change(1,index)" :class="{'active':queryData[1]==index}">{{item.label}}</a>-->
                    <!--</div>-->
                  </li>
                  ...
                  <li class="selitems">
                    ...
                    <div class="sels">
                      ...
                      <a class="sel fix" v-for="(item,index) in selData.priceData" @click="change(2,index+1)" :class="{'active':queryData[2]==index+1}">{{item.label}}</a>
                    </div>
                  ...
                  <!--<li class="selitems">-->
                    ...
                    <!--<div class="sels">-->
                      ...
                      <!--<a class="sel fix" v-for="(item,index) in selData.tarData" @click="change(3,index+1)" :class="{'active':queryData[3]==index+1}">{{item.label}}</a>-->
                  ...
                  <li class="selitems">
                    ...
                    <div class="sels">
                      ...
                      <a class="sel fix" v-for="(item,index) in selData.type1Data" @click="change(4,index+1)" :class="{'active':queryData[4]==index+1}">{{item.label}}</a>
                  ...
                  <!--<li class="selitems" >-->
                    ...
                    <!--<div class="sels">-->
                      ...
                      <!--<a class="sel fix" v-for="(item,index) in selData.type2Data" @click="change(5,index+1)" :class="{'active':queryData[5]==index+1}">{{item.label}}</a>-->
                    <!--</div>-->
                  <!--</li>-->
                  <!--<li class="selitems"  >-->
                    ...
                    <!--<div class="sels">-->
                      ...
                      <!--<a class="sel fix" v-for="(item,index) in selData.hCountData" @click="change(6,index+1)" :class="{'active':queryData[6]==index+1}">{{item.label}}</a>-->
                    <!--</div>-->
                  <!--</li>-->
                  <!--<li class="selitems" >-->
                    <!--<span class="itemlabel">房源特色:</span>-->
                    <!--<div class="sels">-->
                      <!--<a class="sel" :class="{'active':!queryData[7].includes(1)}" @click="changeAllCheck">全部</a>-->

                      <!--<div class="sel fix checkbox" v-for="(item,index) in selData.favData" @click="changeCheck(index)" :class="{'active':queryData[7][index]==1}">-->
   ...
   <div class="wrap2">
      <div class="wrapbox2">
        ...
        <ul class="salelist">
            <li class="saleitem" v-for="(item,index) in list" :key="index" @click="$router.push('/immigrant/detail?id='+item.id)">
              <div class="imgbox" :style="'background-image:url('+IMG_URL+item.projectImagePath+')'">
                  <div class="img-sell"><span>{{item.projectStatus|sellState}}</span></div>
              </div>
              <div class="contentbox">
                  <div><router-link :to="'/immigrant/detail?id='+item.id" class="title">{{item.projectName}}</router-link></div>
                  <div class="detailbox">
                      <div class="item" style="letter-spacing: 2px" v-if="item.categories!=null">办理周期:{{item. projectPeriod}} {{item.projectPeriodUnit}}</div>
                      <div class="item" v-if="item.houseType.length>0">移民类型: {{item.houseType}}</div>
                      <div class="item">项目优势1: {{item.advantage1}}</div>
                      <div class="item">项目优势2: {{item.advantage2}}</div>
                  </div>
                <div class="connect_box">
                  <div class="item">总价:<span class="price">{{item.minPriceAmt|cut}}</span> {{item.priceUnit}}起  {{item.priceCcy}}  </div>
                  ...
                  <div class="item"><a class="collectbtn"  :class="{'disabled':!item.clickable}" @click="collect($event,item.id,item.clickable,index)">{{item.clickable?"收藏":"取消收藏"}}</a></div>
...
<script>
    ...
    import {getProjectCondition,getNation,list,getCityByPid,priceList,typeList} from '@/api/house'
    ...
    export default {
        ...
        methods: {
            change(obj,index){
              ...
              store.dispatch('TOGGLE_immigrantList', this.queryData)
            ...
            clearAll(){
                ...
                this.queryData = list
                store.dispatch('TOGGLE_immigrantList',list)
                this.$router.push('/immigrant')
            },
          collect(e,id,c,i){
            e.preventDefault();
            e.stopPropagation();
            ...
            changeCheck(index){
              if(this.queryData[7][index]==0)
                ...
                this.$set(this.queryData[7],index, 0);
            },
            ...
            getNavBarItem(item,index){
                ...
                if(index==7){
                  ...
                  switch(index){
                    case 0:
                      lab="国家:"+this.selData.countryData[item-1].label;
                      break;
                    case 1:
            ...
            getProCon(){
                getProjectCondition().then(res=>{
                  // console.log(res);
                  // this.selData.priceData=res.body[0].projectHousePriceTypeList.map(item=>{
                ...
                this.selData.type2Data=res.body[0].projectHousingTypesList.map(item=>{
                  return {val:item.id,label:item.typeName};
                });
                ...
                this.selData.favData=res.body[0].projectHousingFeatureList.map(item=>{
                  return {val:item.id,label:item.feature};
                });

                getNation(0).then(res=>{
                    // console.log(res)
                    let nation = this.$route.query.nation;
                    let city = this.$route.query.city;
                    this.selData.countryData=res.body.map(item=>{

                      return {val:item.id,label:item.name,nationCode:item.nationCode};
                    });
            ...
            config.nationId=this.queryData[0]==0?"":this.selData.countryData[this.queryData[0]-1].val;
            config.nationCityId=this.queryData[1]==-1?"":this.selData.stateData[this.queryData[1]].val;
            config.housePriceType=this.queryData[2]==0?"":this.selData.priceData[this.queryData[2]-1].label;
            config.homeGoalIds=this.queryData[3]==0?[]:[this.selData.tarData[this.queryData[3]-1].val];
            config.categoryIds=this.queryData[4]==0?[]:[this.selData.type1Data[this.queryData[4]-1].val];
            config.housingTypes=this.queryData[5]==0?"":this.selData.type2Data[this.queryData[5]-1].label;
            config.houseType=this.queryData[6]==0?"":this.selData.hCountData[this.queryData[6]-1].label;
        ...
        mounted(){
          ...
          this.queryData = store.state.commn.immigrantList

            this.getProCon();
            priceList().then(res=>{
              this.selData.priceData=res.body.map(item=>{
                  return {val:item.id,label:item.typeName};
              });
          })

            typeList().then(res=>{
              this.selData.type1Data=res.body.map(item=>{
                return {val:item.id,label:item.name};
              });
            })
                  ...
                  .sels{
                    ...
                    .checkbox{
                      ...
                      &.active{
                        color: #333;
                        .check_icon{
                          background: url(../../assets/img/custom_checkbox_selected.png) no-repeat;
                        }
                      }
